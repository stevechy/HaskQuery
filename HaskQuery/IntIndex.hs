...
intIndexSelectorM :: (Monad m) => IntIndex -> Int -> Control.Monad.Trans.Cont.Cont (b -> m b) Int
intIndexSelectorM index indexValue =
    let indexSet = Data.IntMap.Strict.findWithDefault (Data.IntSet.empty) indexValue (relationIndex index)
        in Control.Monad.Trans.Cont.cont (\continuation -> (\seed -> Data.IntSet.foldl' (\foldSeed rowId -> foldSeed >>= continuation rowId) (return seed) indexSet ))
